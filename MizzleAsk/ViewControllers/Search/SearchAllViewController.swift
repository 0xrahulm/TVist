    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if dataArray.count > 0{
            ...
            if let selectedCell = tableView.cellForRow(at: indexPath) as? SearchTableViewCell {
                ...
                parentVC?.selectedSearchItem(searchItem: data, loadedImage: selectedCell.itemImage.image)
            }
...
extension SearchAllViewController: SearchTableViewCellProtocol {
    func didTapOnCell(cell: SearchTableViewCell) {
        if let indexPath = self.tableView.indexPath(for: cell) {
            if dataArray.count > 0{
                
                let data = dataArray[indexPath.row]
                parentVC?.selectedSearchItem(searchItem: data, loadedImage: cell.itemImage.image)
